...
<div class="container">
    ...
    <div class="card mb-4" style="border-radius: 14px; box-shadow: 0 8px 25px rgba(30,58,138,0.12); border: none;">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between" style="padding: 1.2rem 1.5rem;">
            <div class="d-flex align-items-center mb-2 mb-md-0">
                ...
                <div>
                    <div style="font-size: 1.3rem; font-weight: bold; font-family: 'Poppins', sans-serif; color: #222;">Create New Cluster</div>
                    <div style="font-size: 0.95rem; font-family: 'Poppins', sans-serif; color: #555; font-weight: 500;">Add a new cluster under a track</div>
                </div>
            </div>
    ...
    <div class="card" style="border-radius: 12px;">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.clusters.store') }}">
                @csrf
                    ...
                    <select class="form-select @error('track_id') is-invalid @enderror" 
                        ...
                        @foreach($tracks as $trackOption)
                            <option value="{{ $trackOption->id }}" 
                                    {{ old('track_id', $trackId) == $trackOption->id ? 'selected' : '' }}>
                                {{ $trackOption->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="form-text text-muted">e.g., STEM, HUMSS, ABM, GAS</small>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-2"></i>Create Cluster
                    </button>
                </div>
            </form>
        </div>
